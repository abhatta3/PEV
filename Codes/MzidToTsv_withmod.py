...
#             PepEvID[line[15]] = [line[5]+'.'+PepID[line[11]]+'.'+line[3],ProtID[line[13]]]
        ...
        if ' experimentalMassToCharge=' in line:
            ...
            Charge[-1].append(int(line[column_parameters['experimentalMassToCharge']['chargeState']]))
            #pep = line[5]
            continue
        if line[0].find('<PeptideEvidenceRef peptideEvidence_ref=')>-1:
            Peptide[-1].append(PepEvID[line[1]][0])
            Protein[-1].append(PepEvID[line[1]][1])
        if 'MS-GF:RawScore' in line:
            if not column_parameters.has_key('MS-GF'):
                column_parameters['MS-GF'] = {'value':FindColumnIndex(line,'value=')}
            MSGFScore[-1].append(int(line[column_parameters['MS-GF']['value']]))
            continue
        if 'MS-GF:DeNovoScore' in line:
            if not column_parameters.has_key('MS-GF'):
                column_parameters['MS-GF'] = {'value':FindColumnIndex(line,'value=')}
            DeNovoScore[-1].append(int(line[column_parameters['MS-GF']['value']]))
            continue
        if 'MS-GF:SpecEValue' in line:
            if not column_parameters.has_key('MS-GF'):
                column_parameters['MS-GF'] = {'value':FindColumnIndex(line,'value=')}
            SpecEValue[-1].append(float(line[column_parameters['MS-GF']['value']]))
            continue
        if 'MS-GF:EValue' in line:
            if not column_parameters.has_key('MS-GF'):
                column_parameters['MS-GF'] = {'value':FindColumnIndex(line,'value=')}
            EValue[-1].append(float(line[column_parameters['MS-GF']['value']]))
            continue
        if 'IsotopeError' in line:
            ...
            IsotopeError[-1].append(int(line[column_parameters['IsotopeError']['value']]))
            continue
    ...
    return [SpecFile,SpecID,FragMethod,Precursor,IsotopeError,Charge,Peptide,Protein,
            DeNovoScore,MSGFScore,SpecEValue,EValue,hash_key]
...
def writeTsv(series,Tsv_file):
    ...
    for index in range(len(series[1])):
        for index2 in range(len(series[11][index])):
            ...
            #if len(series[4][index]) != len(series[6][index]):
                #print series[4][index], series[6][index],series[1][index]
            s.write(str(series[4][index][index2])+'\t') #Isotope
            s.write('NA'+'\t') #PrecursorError
            s.write(str(series[5][index][index2])+'\t') #charge
            s.write(str(series[6][index][index2])+'\t') #Peptide
            s.write(str(series[7][index][index2])+'\t') #Protein
            s.write(str(series[8][index][index2])+'\t') #DeNovoScore
            s.write(str(series[9][index][index2])+'\t') #MSGFScore
            s.write(str(series[10][index][index2])+'\t') #SpecEvalue
            s.write(str(series[11][index][index2])+'\n') #EValue
            #s.write(str(series[12][index][index2])+'\t') #QValue
            #s.write(str(series[13][index][index2])+'\n') #PepQvalue
